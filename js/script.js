    ...
    rpcUrls: ["https://sepolia.infura.io"], // Replace with your Infura/Alchemy RPC URL
    blockExplorerUrls: ["https://sepolia.etherscan.io"]
};
        const CONTRACT_ADDRESS = "0x069c11c4b12B78b5a8a0B0a5B076CAC659381759";
    ...
    try {
        ...
        const xmonoCount = (await contract.methods.claimCount(accounts[0]).call()).toString();
...
// Define TARGET_GAS_COST higher for zkVerify to ensure sufficient gas for complex operations like ZK proof verification
const TARGET_GAS_COST = web3.utils.toWei('0.05', 'ether'); // Increased from ~0.01 to 0.05 ETH for higher final gas cost
...
try {
    ...
    // Adjust gasLimit to meet ~0.05 ETH if needed, with higher buffer for zkVerify
    if (currentGasCost < TARGET_GAS_COST) {
        gasLimit = Math.floor(TARGET_GAS_COST / gasPrice * 1.5); // Increased buffer to 50% for safety
        console.log("Adjusted Gas Limit to meet 0.05 ETH:", gasLimit);
    } else {
        gasLimit = Math.floor(gasEstimate * 1.5); // Increased buffer to 50% for safety
        console.log("Gas Limit (with buffer):", gasLimit);
    }
} catch (error) {
    console.error("Gas Estimation Error:", error);
    addLogEntry(`Gas estimation failed: ${error.message}`, 'loss');
    // Fallback to a higher gas limit suitable for ZK proof verification on zkVerify
    gasLimit = 5000000; // Increased fallback for complex operations
    console.log("Using fallback Gas Limit:", gasLimit);
}

// Get gas price (use network average or set a minimum higher for zkVerify)
let gasPrice = await web3.eth.getGasPrice();
const MIN_GAS_PRICE = web3.utils.toWei('50', 'gwei'); // Increased minimum to 50 Gwei for faster/more reliable processing
if (gasPrice < MIN_GAS_PRICE) {
    gasPrice = MIN_GAS_PRICE;
    console.log("Using minimum Gas Price:", web3.utils.fromWei(gasPrice, 'gwei'), "gwei");
} else {
    // Optionally boost gas price by 20% if above min for better success rate
    gasPrice = Math.floor(gasPrice * 1.2);
    console.log("Boosted Gas Price:", web3.utils.fromWei(gasPrice, 'gwei'), "gwei");
}
        ...
        async function updateUI() {
            if (accounts.length > 0) {
                ...
                try {
                    ...
                    const xmonocount = await contract.methods.claimCount(accounts[0]).call();
                    document.getElementById('xmonoCount').textContent = xmonocount;
